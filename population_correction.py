...
person_wd = os.getcwd() + \
    "\\AMELIA\\AMELIA_P_level_v0.2.3 (Person-Level)\\AMELIA_P_level_v0.2.3\\"
...
def KNN(Target, Classes):
    ...
    for i in tqdm(range(len(Target))):
        ...
        NN = Classes.iloc[abs(Classes.loc[Classes['Province'] == Target['Province'][i]]
                              ['Person_Income'] - Target.iloc[i]['Person_Income']).idxmin(axis=0)]['City/Community']
...
Check_Region = Trimmed_Age.groupby(
    ["City/Community", "Province"]).size().reset_index().rename(columns={0: "Count"})

# Generate our Target and Class Dataframes for KNN
Target_list = Check_Region.loc[(Check_Region["Count"] < 300)]
Classes = Trimmed_Age.groupby(["City/Community", "Province"]
                              )["Person_Income"].agg(Person_Income='mean').reset_index().round(2)

# Create Dataframe for our Target Cities
Target = Classes.merge(Target_list, left_on='City/Community', right_on='City/Community').drop(
    columns="Province_y").rename(columns={"Province_x": "Province"})
...
Districts = Trimmed_Age.groupby(
    ['City/Community', 'District']).size().reset_index()
...
print(Trimmed_Age.groupby(["City/Community", "Province", 'District']
                          ).size().reset_index().rename(columns={0: "Count"}).head())
...
# Index reset is requirement for export
Trimmed_Age.reset_index().to_feather(person_wd + "AMELIA.feather")
