...
resample <- function(sample_diff, data, smoteR = F, use_relevance = F, thresh = NULL){
  ...
  #'       - Oversample if difference in density between sample and AMELIA is > 0, 
  #'         undersample if < 0
  #'       - Compute amount of necessary sampling as the difference in proportions
  #'       - Scale by some weight that grows very fast, as differences in proportions 
  #'         get much smaller for very high income rates due to very few cases overall
  ...
  if (!use_relevance) weights <- (1:length(buckets))^2
  else weights <- (1:length(buckets))^2
  ...
  for (i in 2:length(buckets)){
    ...
    if (!use_relevance) sample_proportion <- proportions[i-1] * weights[i-1]
...
test <- resample(sample_difference, data = resample_data)
sample_difference <- compute_income_diff(ex_sample$`Stratified Cluster`$Base$Person_Income, plot = T)
test_income_diff <- compute_income_diff(test$Person_Income, plot = T)
